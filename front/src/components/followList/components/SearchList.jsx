...
export default function SearchList(props) {
  ...
  function getFriend(event) {
    ...
    let result = {
      followerInfo: firstFriendList.followerInfo.filter(data => {
  ...
  return (
    <>
      <SearchForm>
        ...
        <FriendList>
          {!friendList.followerInfo || friendList.followerInfo.length === 0 ? (
            <div>일치하는 친구가 없습니다.</div>
          ) : (
            friendList.followerInfo &&
            friendList.followerInfo.map((follow, index) => (
